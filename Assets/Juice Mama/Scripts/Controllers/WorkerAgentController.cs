...
public class WorkerAgentController : AgentController
{
    ...
    void RefreshTrees(bool force)
    {
        ...
        for (int i = 0; i < objs.Length; i++)
        {
            ...
            {
                var f = any[k];
                if (f.GetFruitData() == job.fruitData) { match = true; break; }
            }
    ...
    bool HasGrownFruit(Transform t)
    {
        ...
        {
            var f = fruits[i];
            if (f.GetFruitData() == job.fruitData && f.isGrown && f.gameObject.activeInHierarchy) return true;
        }
        return false;
    }
    ...
    bool TryPickFruit()
    {
        ...
        {
            var fruit = fruits[i];
            if (fruit.GetFruitData() == job.fruitData && fruit.isGrown && (storage.GetFruitCount() < maxFruitCapacity))
            {
